...
export const UsuariosLista = () => {
  ...
  const getUsuarios = async () => {
    ...
    const sesion = await axios.get('/api/sesiones/' + codigo_sesion)
      .then(res => { setSesion({ nombre_sesion: res.data.message[0].nombre_sesion }) })
      .catch(error => { setCategorias([]) })



      const users = await axios.get('/api/sesiones/usuarios_x_quest/' + codigo_sesion)
  ...
  const onUsersUpload = async (event) => {
    ...
    const res = await axios.post('/api/asignaciones/repartir/', 
    {
        codigo_sesion: codigo_sesion
    });
